...
/**
 * Retorna os responsáveis vinculados a uma criança específica.
 */
export async function checkChildLinkedGuardians(
  childId: string
): Promise<{ id: string; full_name: string; relationship: string }[]> {
  try {
    const { data, error } = await supabase
      .from("guardian_children")
      .select("guardian_id, relationship, guardian:guardians(id, full_name)")
      .eq("child_id", childId)

    if (error || !data) return []

    const guardians: { id: string; full_name: string; relationship: string }[] = []
    data.forEach((item: any) => {
      if (item.guardian && item.guardian.id) {
        guardians.push({
          id: item.guardian.id,
          full_name: item.guardian.full_name,
          relationship: item.relationship || "Responsável",
        })
      }
    })
    return guardians
  } catch (err) {
    console.error("Erro ao verificar responsáveis da criança:", err)
    return []
  }
}
 ...
 * 3. Se solicitado (deleteGuardianIds), exclui também os responsáveis selecionados caso não tenham outros filhos.
 */
export async function deleteChildSafely(
  ...
  try {
    ...
    // 5. Se o usuário escolheu excluir também os responsáveis vinculados
    if (deleteGuardianIds && deleteGuardianIds.length > 0) {
      for (const gId of deleteGuardianIds) {
        // Verificar se o responsável ainda tem outras crianças vinculadas
        ...
        if (!otherLinks || otherLinks.length === 0) {
          // Não tem mais nenhum outro filho, pode excluir o responsável com segurança
          await supabase.from("guardians").delete().eq("id", gId)
        }
      }
    }
 ...
 * 2. Se solicitado (deleteChildIds), exclui com segurança cada criança selecionada (preservando financeiro).
 * 3. Exclui o registro da tabela guardians.
 */
export async function deleteGuardianSafely(
  ...
  try {
    // 1. Se o usuário escolheu excluir também as crianças vinculadas
    if (deleteChildIds && deleteChildIds.length > 0) {
      for (const cId of deleteChildIds) {
        await deleteChildSafely(cId, profId)
      }
    }

    // 2. Remover todos os vínculos em guardian_children
    await supabase
      .from("guardian_children")
      .delete()
      .eq("guardian_id", guardianId)

    // 3. Excluir o responsável
    const { error: deleteError } = await supabase
      .from("guardians")
      .delete()
      .eq("id", guardianId)
